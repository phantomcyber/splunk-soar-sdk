"""OAuth 2.0 client implementation using httpx.

This module provides the core OAuth client that handles token acquisition,
...
class SOARAssetOAuthClient:
    ...
    def __init__(
        ...
        *,
        ...
        # Force IPv4 to avoid DNS resolution issues in SOAR environment
        self._http_client = http_client or httpx.Client(
            transport=httpx.HTTPTransport(verify=verify_ssl, local_address="0.0.0.0"),
    ...
    def _load_state(self, *, force_reload: bool = False) -> OAuthState:
        """Load OAuth state from asset storage."""
        state_data = self._auth_state.get_all(force_reload=force_reload)
        if not state_data:
            return OAuthState()
    ...
    def get_stored_token(self, *, force_reload: bool = False) -> OAuthToken | None:
        ...
        state = self._load_state(force_reload=force_reload)
        return state.token
    ...
    def fetch_token_with_authorization_code(
        ...
        *,
        ...
        state = self._load_state(force_reload=True)
        ...
        try:
            ...
            error_detail = self._extract_error_detail(e.response)
            raise OAuthClientError(
                f"Authorization code token request failed: {error_detail}"
            ) from e

        except httpx.RequestError as e:
            raise OAuthClientError(
                f"Authorization code token request failed: {e}"
            ) from e
    ...
    def get_authorization_code(self, *, force_reload: bool = True) -> str | None:
        """Get stored authorization code from state."""
        state = self._load_state(force_reload=force_reload)
        if state.session and state.session.auth_code:
...
class CertificateOAuthClient(SOARAssetOAuthClient):
    ...
    def fetch_token_with_certificate(
        ...
        *,
        ...
        now = int(calendar.timegm(datetime.now(UTC).utctimetuple()))
        jwt_payload = {
