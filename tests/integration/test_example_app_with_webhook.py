...
def app_info(phantom_instance):
    """Get the example app with webhook info."""
    app_name = "example_app"  # This is the "name" field in app.json (both apps have same name)
    app_vendor = "Splunk Inc."
    app_info_result = phantom_instance.get_app_info(name=app_name, vendor=app_vendor)

    # Since both apps have the same name, we need to find the webhook version
    # by checking for webhook-specific features
    assert app_info_result["count"] > 0, f"App '{app_name}' not found on instance"

    # For now, just use the first one we find
    # In production, you might need additional logic to differentiate
    return app_info_result["data"][0]


@pytest.fixture(scope="module")
def asset_config():
    """Create asset configuration for webhook app."""
    return {
...
def asset_id(phantom_instance, app_info, asset_config):
    """Create and configure an asset for the example app with webhook."""
    asset_name = f"example_app_webhook_integration_test_asset"
...
def test_container(phantom_instance):
    ...
    container_id = phantom_instance.create_container(
        container_name="Integration Test Container - Webhook",
        label=label,
        tags=["integration_test", "sdk", "webhook"],
    )
...
def test_connectivity(phantom_instance, app_info, asset_id, test_container):
    ...
    action_id = phantom_instance.run_action(
        ...
        targets=targets,
        name="integration_test_connectivity_webhook",
    )
