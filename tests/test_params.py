...
def test_make_request_params_validation():
    ...
    assert (
        str(e.value)
        == "MakeRequestParams subclass 'BrokenMakeRequestParams' can only define these fields: ['body', 'endpoint', 'headers', 'http_method', 'query_params', 'timeout', 'verify_ssl']. Invalid fields: ['not_allowed']"
    )


def test_make_request_params_subclass_schema():
    class MakeRequestParamsSubclass(MakeRequestParams):
        query_params: str = Param(description="Query parameters for virustotal")

    assert (
        MakeRequestParamsSubclass._to_json_schema()["query_params"]["description"]
